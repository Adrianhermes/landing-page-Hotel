...
async function handleSaibaMais(button) {
  ...
  try {
    if (STATIC_SITE_MODE) {
      ...
      if (infoQuarto) infoQuarto.textContent = nome;
      if (infoPeriodo) infoPeriodo.textContent = formatDateForHuman(hojeIso) + " ate " + formatDateForHuman(amanhaIso);
      if (infoStatus) infoStatus.textContent = "Disponivel";
      if (infoCriada) infoCriada.textContent = formatDateTimeForHuman(hoje.toISOString().slice(0, 19).replace("T", " "));
      if (infoMensagem) {
        infoMensagem.textContent = "Versao demonstracao: esta pagina estatica nao cria reservas no servidor.";
      }
      return;
    }
    ...
    try {
      response = await fetch("./controllers/criarReservaRapida.php", {
        method: "POST",
...
if (reservaForm && reservaFeedback && reservaModalCheckin && reservaModalCheckout) {
  reservaForm.addEventListener("submit", (event) => {
    ...
    if (STATIC_SITE_MODE) {
      ...
      alert("Reserva enviada em modo demonstracao. Em deploy estatico, os dados nao sao gravados.");
    }
  });
}
...
if (infoReservasButton) {
  infoReservasButton.addEventListener("click", () => {
    ...
    window.location.href = "./controllers/liberarReservaPublica.php";
  });
}
